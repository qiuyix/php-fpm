...
# 安装xdebug
RUN cd /tmp && curl -Ok https://xdebug.org/files/xdebug-3.1.6.tgz && tar -zxvf /tmp/xdebug-3.1.6.tgz && cd xdebug-3.1.6 && phpize && ./configure && make && make install && echo "zend_extension=xdebug.so" >>  /usr/local/etc/php/conf.d/docker-php-ext-xdebug.ini && rm -rf /tmp/* /var/tmp/*
